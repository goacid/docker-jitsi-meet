...
function Util:get_public_key(keyId)
	...
	if string.sub(self.asapKeyServer, 0, 5) ~= "https" and string.sub(self.asapKeyServer, 0, 4) then
            module:log("debug", "PEM file server is not an URL");
	    local pathFile = self.asapKeyServer .. hex.to(sha256(keyId)) .. '.pem';
	    module:log("debug", "Looking for localfile : " .. pathFile);
	    local fileFound = io.open(pathFile, "rb");
	    if fileFound == nil then
                module:log("debug", "    File not found !");
		return nil;
            else
                module:log("debug", "    File found and opened");
                local content = fileFound:read("*all");
                fileFound:close();
                return content;
            end
