...
if [[ $DISABLE_HTTPS -ne 1 ]]; then
  if [[ $ENABLE_LETSENCRYPT -eq 1 ]]; then
    if [[ ! -f /config/letsencrypt/live/$LETSENCRYPT_DOMAIN/fullchain.pem ]]; then
      if ! certbot \
        certonly \
        --no-self-upgrade \
        --noninteractive \
    ...
    if [[ ! -f /etc/periodic/weekly/letsencrypt-renew ]]; then
